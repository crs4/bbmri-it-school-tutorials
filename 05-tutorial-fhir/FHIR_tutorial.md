...
We will take the Specimen resource as an example. Let's identify the structure of the Specimen resource in 
version 4.0.1 of FHIR, by looking at the official FHIR specification:
https://www.hl7.org/fhir/Specimen.html

Look at the documentation page of the Specimen resource above and try to identify: 
...
learn how to interact with the FHIR API, and to make some CRUD operations. To do that, we will use as FHIR server
the one provided by BBMRI.de, that is the same used by the Sample Locator (we will talk a lot about it in 
future lessons).

First, we will run the docker compose to start the Blaze FHIR server. From ./blaze, run:
```bash
docker compose up -d
```
This will start the FHIR server. We will use Postman, that we also used in the previous tutorial, to interact with it.
Open Postman and on the left side, near the "Collections" tab, click on the "+" button to create a new collection. 
...
create the first GET request, by selecting "Add request" in the appearing panel. Name it 
...
At this point the  server is empty, it means that it does not contain any resource yet. We'll now add some resources, through 
some POST requests. The ./resources folder contains the resources to add. We will add some patients and some 
related specimens. 

## Creating and retrieving FHIR resources (POST and GET)
Now, in the same way as we did for the capability statement, let's create a new request in the same Postman collection, named
...
The setup of the POST request should appear like this:
...
- As explained on the slide 13 of the lessons, pay attention to the identifiers in the response:
    - a) the id "DGBRFSQE7HY6IWA2" is the unique identifier assigned by the server to the resource. You'll
         notice that yours in your own response is different from this one. This id is the one that must
    ...
    - b) the identifier "patient-01" is instead the value of the identifier attribute that is part of the 
         patient resource, that has been assigned by us. It will remain the same in every instance of the 
         Blaze FHIR server where this patient resource is created.
...
Now, let's add two specimens, each one related to one of the two patients. The patient's reference must 
be set in the Specimen resource by properly valuing the  "subject" attribute (we are referring to FHIR version 4.0.1). 
...
```
The value of yout patient 01 resource will vbe different, as it is assigned by your own server instance.

If all went fine, you should see a response like this:

![Get Patient 01](./images/get_patient_01.png)

Now, do the same for the second specimen (specimen-02), relating it to the second patient (patient-02).
...
You can also check the capability statement with a GET on: 
...
Remember that each key name in the dictionary must match the name of the search parameter defined in the
PDQm profile. You can add or remove parameters by adding or removing keys in the dictionary.
...
> **_Exercise:_**  improve the previous code to parse the output, get the next link and use it to perform a new
...
and the publisher script when it starts, so we can run it and generate the IG:
...
> **_Exercise:_**  now, try to add a new extension to the profile, for example an extension to indicate the 
